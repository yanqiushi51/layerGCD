...
**Remote sensing GCD (AID/NWPU)**

The local adaptation also supports the project remote-sensing protocol for AID and NWPU-RESISC45. Dataset roots default to ``/home/yqs/redar/data/AID`` and ``/home/yqs/redar/data/NWPU-RESISC45``; override the parent directory with ``DATASET_DIR`` if needed.
...
Use ``--dataset_name nwpu`` for NWPU-RESISC45, and ``--split_type confusable`` for the fixed confusable split.
