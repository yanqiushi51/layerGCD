...
def _find_class_dirs(root, class_names):
    ...
    if not os.path.isdir(root):
        raise FileNotFoundError("Remote sensing dataset root does not exist: {}".format(root))
    ...
    raise RuntimeError(
        "Could not match all class folders under {}. Missing: {}. "
        "Folder names may use spaces, underscores, hyphens, or CamelCase.".format(root, missing)
    )
...
class RemoteSensingSceneDataset(Dataset):
    def __init__(self, root, class_names, include_classes=None, transform=None,
        ...
        if not samples:
            raise RuntimeError("No images found for classes {} in {}".format(
                sorted(self.include_classes), self.root))
...
def get_remote_sensing_class_splits(dataset_name, split_type="random", seed=0):
    ...
    if split_type == "random":
        ...
        raise ValueError("Unknown split_type '{}'. Use 'random' or 'confusable'.".format(split_type))

    if len(set(old) & set(novel)) > 0:
        raise RuntimeError("{} {} split has overlapping old/novel classes".format(
            dataset_name, split_type))
    if sorted(old + novel) != list(range(len(classes))):
        raise RuntimeError("{} {} split does not cover all classes".format(dataset_name, split_type))
