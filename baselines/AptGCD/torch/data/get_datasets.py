...
from data.remote_sensing import get_remote_sensing_class_splits, get_remote_sensing_datasets
...
supported_datasets = (
    'cifar10', 'cifar100', 'imagenet_100', 'imagenet_1k', 'herbarium_19',
    'cub', 'aircraft', 'scars', 'aid', 'nwpu',
)


def get_dataset_func(dataset_name):
    if dataset_name == 'cifar10':
        from data.cifar import get_cifar_10_datasets
        return get_cifar_10_datasets
    elif dataset_name == 'cifar100':
        from data.cifar import get_cifar_100_datasets
        return get_cifar_100_datasets
    elif dataset_name == 'imagenet_100':
        from data.imagenet import get_imagenet_100_datasets
        return get_imagenet_100_datasets
    elif dataset_name == 'imagenet_1k':
        from data.imagenet import get_imagenet_1k_datasets
        return get_imagenet_1k_datasets
    elif dataset_name == 'herbarium_19':
        from data.herbarium_19 import get_herbarium_datasets
        return get_herbarium_datasets
    elif dataset_name == 'cub':
        from data.cub import get_cub_datasets
        return get_cub_datasets
    elif dataset_name == 'aircraft':
        from data.fgvc_aircraft import get_aircraft_datasets
        return get_aircraft_datasets
    elif dataset_name == 'scars':
        from data.stanford_cars import get_scars_datasets
        return get_scars_datasets
    elif dataset_name in ('aid', 'nwpu'):
        return get_remote_sensing_datasets
    else:
        raise ValueError
...
def get_datasets(dataset_name, train_transform, test_transform, args):
    ...
    # Get datasets
    get_dataset_f = get_dataset_func(dataset_name)
...
def get_class_splits(args):
    ...
    # GET CLASS SPLITS
    # -------------
    if args.dataset_name in ('aid', 'nwpu'):

        args.image_size = 224
        split_type = getattr(args, 'split_type', 'random')
        split_seed = getattr(args, 'class_split_seed', 0)
        args.train_classes, args.unlabeled_classes = get_remote_sensing_class_splits(
            args.dataset_name,
