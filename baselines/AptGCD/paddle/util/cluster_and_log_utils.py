...
sys.path.append("/data/zbp05/ZW/aptgcd_paddle/util")
import numpy as np
import paddle
...
def all_sum_item(item,device='gpu:4'):
    item = paddle.to_tensor(data=item).to(device,blocking=True)
    paddle.distributed.all_reduce(tensor=item)
    return item.item()
...
def split_cluster_acc_v2(y_true, y_pred, mask):
    ...
    # D = max(y_pred.max_func(), y_true.max_func()) + 1
    ...
    try:
        if paddle.distributed.get_world_size() > 0:
...
def split_cluster_acc_v2_balanced(y_true, y_pred, mask,device='gpu:4'):
    """
    Calculate clustering accuracy. Require scikit-learn installed
    First compute linear assignment on all data, then look at how good the accuracy is on subsets
    ...
    for i in range(y_pred.size):
        w[y_pred[i], y_true[i]] += 1
    ind = linear_assignment(w.max_func() - w)
    ind = np.vstack(ind).T
    ind_map = {j: i for i, j in ind}
    ...
    try:
        if paddle.distributed.get_world_size() > 0:
            old_acc, new_acc = paddle.to_tensor(data=old_acc).to(device,
                blocking=True
            ), paddle.to_tensor(data=new_acc).to(device,blocking=True)
            paddle.distributed.all_reduce(
                tensor=old_acc
            ), paddle.distributed.all_reduce(tensor=new_acc)
            paddle.distributed.all_reduce(
                tensor=total_old_instances
            ), paddle.distributed.all_reduce(tensor=total_new_instances)
            old_acc, new_acc = old_acc.cpu().numpy(), new_acc.cpu().numpy()
            total_old_instances, total_new_instances = (
                total_old_instances.cpu().numpy(),
                total_new_instances.cpu().numpy(),
            )
    except:
        pass
    total_acc = np.concatenate([old_acc, new_acc]) / np.concatenate(
        [total_old_instances, total_new_instances]
    )
    old_acc /= total_old_instances
    new_acc /= total_new_instances
...
EVAL_FUNCS = {"v2": split_cluster_acc_v2, "v2b": split_cluster_acc_v2_balanced}


def log_accs_from_preds(
    y_true, y_pred, mask, eval_funcs, save_name, T=None, print_output=True, args=None
):
    """
    ...
    for i, f_name in enumerate(eval_funcs):
        ...
        args.writer.add_scalars(
            log_name, {"Old": old_acc, "New": new_acc, "All": all_acc}, T
        )
    return to_return
