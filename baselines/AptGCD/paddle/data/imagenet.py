...
paddle.vision.datasets.ImageFolder(root="/data/zbp05/ZW/dataset/imagenet/val")
...
class ImageNetBase(paddle.vision.datasets.ImageFolder):
...
def get_imagenet_100_datasets(
    ...
    imagenet_training_set = ImageNetBase(
        root=os.path.join(imagenet_root, "train"), transform=train_transform
    )
    whole_training_set = subsample_classes(
        imagenet_training_set, include_classes=subsampled_100_classes
    )
    whole_training_set.samples = [
        (s[0], cls_map[s[1]]) for s in whole_training_set.samples
    ]
    ...
    test_dataset = ImageNetBase(
        root=os.path.join(imagenet_root, "val"), transform=test_transform
    )
    test_dataset = subsample_classes(
        test_dataset, include_classes=subsampled_100_classes
    )
...
def get_imagenet_1k_datasets(
    ...
    np.random.seed(seed)
    whole_training_set = ImageNetBase(
        root=os.path.join(imagenet_root, "train"), transform=train_transform
    )
    train_dataset_labelled = subsample_classes(
        deepcopy(whole_training_set), include_classes=train_classes
...
if __name__ == "__main__":
    ...
    print(
        set.intersection(
            set(x["train_labelled"].uq_idxs), set(x["train_unlabelled"].uq_idxs)
        )
    )
    print("Printing total instances in train...")
    print(
        len(set(x["train_labelled"].uq_idxs)) + len(set(x["train_unlabelled"].uq_idxs))
    )
