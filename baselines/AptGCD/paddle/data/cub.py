...
class CustomCub2011(paddle.io.Dataset):
    ...
    def __init__(
        ...
        loader=folder.default_loader,  # ✅ 改了这里
        download=False,
    ):
    ...
    def _download(self):
        ...
        torchvision.datasets.utils.download_url(
            self.url, self.root, self.filename, self.tgz_md5
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Num Labelled Classes: {len(set(x['train_labelled'].data['target'].values))}"
    )
    print(
        f"Num Unabelled Classes: {len(set(x['train_unlabelled'].data['target'].values))}"
    )
